...
const SAMPLE_COLUMN_DESCRIPTIONS: Array<[string, string, string]> = [
    ...
    ["instrument_settings_acq_pressure_gain", "Pressure is in centibars for the UVP5 and in decibars (more standard) for the UVP6 in the raw data files. The value is `10` or `0.1` for UVP5 and `1` for UVP6. Since the exported LPM data is in decibars, this field is informational here but kept in the raw export.", "UVP5: constant `0.1` or `10` (TODO, pending Marc). UVP6: constant `1`."],
    ...
    ["instrument_settings_acq_shutter_speed", "Shutter code for UVP5SD (`12` = 100 µs = 1/10000 s).", "UVP5: `work/HDR*.txt` `Exposure` (current — may be swapped with `acq_exposure`, see follow-up #8). UVP6: not currently parsed (empty)."],
    ...
    ["instrument_settings_acq_exposure", "Shutter value in µs (UVP5HD: increased by 60 µs vs the UVPdb 'net' value — important when comparing with UVPdb; UVP6: the value in µs).", "UVP5: `work/HDR*.txt` `ShutterSpeed` (current — may be swapped with `acq_shutter_speed`, see follow-up #8). UVP6: not currently parsed (empty)."],
