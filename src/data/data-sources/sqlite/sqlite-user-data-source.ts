...
export class SQLiteUserDataSource implements UserDataSource {
    ...
    async ensureSeedUser(user: UserSeedModel): Promise<void> {
        const sql = `INSERT OR IGNORE INTO user (first_name, last_name, email, password_hash, valid_email, is_admin, organisation, country, user_planned_usage) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);`;
        const params = [user.first_name, user.last_name, user.email, user.password_hash, 1, 0, user.organisation, user.country, user.user_planned_usage];
