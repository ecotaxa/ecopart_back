// End-to-end test: UVP5 import flow
// Covers samples with images / without images / zipped / unzipped, CTD import,
// and EcoTaxa import on the TEST ecotaxa-dev instance.
//
// Dataset used:
...
const SAMPLE_WITH_IMAGES_UNZIPPED = "omer2_3"
...
const ALL_SAMPLES = [
    ...
    SAMPLE_WITH_IMAGES_ZIPPED,
    SAMPLE_WITH_IMAGES_UNZIPPED,
...
describe("End-to-end: UVP5 import (samples / CTD / EcoTaxa, with and without images)", () => {
    ...
    const testProjectTitle = `TEST_import_UVP5_AUTO_${testRunId}`
    const testProjectAcronym = `UVP5_AUTO_${testRunId}`
    ...
    beforeAll(async () => {
        tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "ecopart_e2e_uvp5_"))
        ...
        db = await new Promise<sqlite3.Database>((resolve, reject) => {
            const database = new sqlite3.Database(path.join(dbFolder, "test_e2e_uvp5.db"), (err) => {
                if (err) reject(err)
    ...
    test("POST /users/ should create a new user", async () => {
        const response = await request(server)
            ...
            .send({
                last_name: "Bancel",
                first_name: "Victoria",
                email: `victoria.bancel+uvp5_${testRunId}@imev-mer.fr`,
                password: "Test1234...",
                organisation: "LOV",
                country: "FR",
    ...
    // ─── Phase 5: Create UVP5 project ────────────────────────────────────
    test("POST /projects/ should create a UVP5 project", async () => {
        ...
        const response = await request(server)
            ...
            .set("Cookie", cookieHeader())
            .send({
                root_folder_path: "remote/ftp_plankton/Ecotaxa_Data_to_import/uvp5_endtoend_TEST",
        ...
        expect(response.body.instrument_model).toBe("UVP5Z")
        expect(response.body.ecotaxa_project_id).toBeDefined()

        capturedProjectId = response.body.project_id
    })

    // ─── Phase 6: Import samples (4 variants: zipped/unzipped × with/without images) ──
    test("POST /projects/:id/samples/import should import 4 samples (mixed images/zipped)", async () => {
    ...
    test("POST /projects/:id/ctd_samples/import should import CTD data for all 4 samples", async () => {
        const listRes = await request(server)
