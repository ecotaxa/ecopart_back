...
describe("PG DataSource", () => {
    ...
    test("getAll", async () => {
        ...
        jest.spyOn(mockDatabase, "all").mockImplementation(() => Promise.resolve({ rows: [{ name: "Smith", id: "123" }] }))
    ...
    //     expect(mockDatabase.query).toHaveBeenCalledWith("delete tb_user where id = $1", ["1"])
    // })

    // test("updateOne", async () => {
    //     const ds = new SQLiteUserDataSource(mockDatabase);
    //     await ds.updateOne("1", { name: "Ramon" });
    //     expect(mockDatabase.query).toHaveBeenCalledWith("update tb_user set name = $1 where id = $2", ["Ramon", "1"])
    // })
    ...
    //     expect(result).toStrictEqual({ name: "Smith", id: "123" })
    //     expect(mockDatabase.query).toHaveBeenCalledWith("select * from tb_user where id = $1 limit 1", ["123"])
    // })
